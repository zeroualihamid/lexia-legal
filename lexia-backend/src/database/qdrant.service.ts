...
export class QdrantService implements OnModuleInit {
  ...
  constructor(private configService: ConfigService) {
    ...
    const https = this.configService.get<boolean>('qdrant.https');
  ...
  async onModuleInit() {
    try {
      ...
      this.logger.warn(
        `Skipping Qdrant collection init: ${err?.message || err}. ` +
          'Check QDRANT_HOST/QDRANT_PORT/QDRANT_API_KEY and that Qdrant is running.',
      );
      return;
    }
