...
export class ChunkingService {
  ...
  private splitByTokens(text: string, startIndex: number): Chunk[] {
    ...
    while (pos < text.length) {
      ...
      // Advance by chunk length minus overlap, but always make forward
      // progress. When the remaining tail is <= OVERLAP_CHARS the naive step
      // is <= 0, which would loop forever (heap OOM), so fall back to the full
      // chunk length to terminate.
      const advance = chunkText.length - this.OVERLAP_CHARS;
      pos += advance > 0 ? advance : chunkText.length;
    }
