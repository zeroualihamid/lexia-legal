...
// Global legal corpus only. User-uploaded documents live in the agent's
// FastEmbed collection (lexia_user_docs, 1024d) and are retrieved via
// AgentDocsClient — never queried here with OpenAI 3072d vectors.
...
Only include collections with score >= 0.3. Score reflects relevance.`,
  ...
  async search(
    ...
    // The global corpus is embedded with OpenAI vectors. If embeddings are
    // unavailable (e.g. the chat LLM is DeepSeek, which has no embeddings API,
    // or the OpenAI key is out of quota) we skip corpus retrieval gracefully —
    // user-document grounding (FastEmbed via the agent) still works.
    let vector: number[];
    try {
      vector = await this.embeddingService.embedText(question);
    } catch (err) {
      this.logger.warn(
        `Corpus embedding unavailable; skipping global corpus search: ${err.message}`,
      );
      return [];
    }
