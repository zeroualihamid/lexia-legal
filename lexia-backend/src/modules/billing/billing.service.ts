  ...
  async getPlans(): Promise<any[]> {
    return this.postgresService.query(
      `SELECT
         ...
         name_fr AS name,
         name_ar,
         price_monthly_mad AS price_monthly,
         price_yearly_mad AS price_yearly,
         ARRAY_REMOVE(ARRAY[
           CASE WHEN max_messages_per_day < 0 THEN 'Messages illimités' ELSE max_messages_per_day || ' messages / jour' END,
           CASE WHEN max_searches_per_day < 0 THEN 'Recherches illimitées' ELSE max_searches_per_day || ' recherches / jour' END,
           CASE WHEN max_uploads_per_month < 0 THEN 'Uploads illimités' ELSE max_uploads_per_month || ' uploads / mois' END,
           CASE WHEN has_sources THEN 'Sources et références' END,
           CASE WHEN has_history THEN 'Historique des conversations' END,
           CASE WHEN has_export THEN 'Export des résultats' END,
           CASE WHEN has_upload THEN 'Documents personnels' END
         ], NULL) AS features,
         json_build_object(
           'messages_per_day', max_messages_per_day,
           'searches_per_day', max_searches_per_day,
           'uploads_per_month', max_uploads_per_month
         ) AS limits
       FROM subscription_plans
       WHERE is_active = true
       ORDER BY price_monthly_mad ASC`,
    );
  }
  ...
  async adminOverview(): Promise<any> {
    const [revenue, subscribers, mrr] = await Promise.all([
      this.postgresService.queryOne<any>(
        `SELECT
           COALESCE(SUM(sp.price_monthly_mad), 0) as total_revenue_mad,
      ...
      this.postgresService.queryOne<any>(
        `SELECT
           COALESCE(SUM(sp.price_monthly_mad), 0) as mrr
         FROM subscriptions s
         JOIN subscription_plans sp ON sp.id = s.plan_id
         WHERE s.status = 'active'`,
      ),
